...
export async function renderMapAdmin() {
    ...
    $('#page-content').classList.add('owner-map-page');
    let type = 'coffee';
    let slots = data.tables.map(item => ({ ...item }));
    const mapIcons = {
        coffee: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 9h13v5a5 5 0 0 1-5 5H9a5 5 0 0 1-5-5V9Z"></path><path d="M17 11h1.5a2.5 2.5 0 0 1 0 5H17"></path><path d="M8 3v3M12 3v3"></path></svg>',
        fishing: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 12c2.4-3.2 5.2-4.8 8.4-4.8 3.3 0 6.1 1.6 8.6 4.8-2.5 3.2-5.3 4.8-8.6 4.8C9.2 16.8 6.4 15.2 4 12Z"></path><path d="m4 12-3-3v6l3-3Z"></path><circle cx="16.5" cy="11" r=".8" fill="currentColor" stroke="none"></circle></svg>',
    };
    ...
    const coffeePreview = () => `<section class="pos-section admin-map-pos-preview">
        ...
        <div class="pos-table-grid">${slots.map(slot => `<button type="button" class="pos-table-card ${slotStateClass(slot)}" data-admin-slot="${slot.id}" aria-label="Chỉnh sửa ${escapeHtml(slot.label)}. ${escapeHtml(coffeeSlotStateLabel(slot))}. ${escapeHtml(coffeeSlotSubtitle(slot))}"><span class="table-state">${coffeeSlotStateLabel(slot)}</span><strong>${escapeHtml(slot.label)}</strong><small>${escapeHtml(coffeeSlotSubtitle(slot))}</small></button>`).join('')}</div>
    </section>`;

    const fishingSlot = (slot, side, row) => {
        const status = fishingSlotStatus(slot);
        return `<button type="button" class="fishing-slot ${slotStateClass(slot)} side-${side}" style="grid-column:${side === 'left' ? 1 : 3};grid-row:${row}" data-admin-slot="${slot.id}" aria-label="Chỉnh sửa ${escapeHtml(slot.label)}. ${escapeHtml(status.label)}. ${escapeHtml(status.detail)}">
    ...
    const openResourceModal = slot => {
        ...
        openModal({
            ...
            onReady(modal, close) {
                modal.classList.add('admin-resource-modal');
                $('#save-admin-resource', modal).onclick = async event => {
                ...
                if (deleteBtn) {
                    deleteBtn.onclick = async () => {
                        if (!await confirmModal(
                            `Xóa ${resourceName}`,
                            `${resourceName[0].toUpperCase() + resourceName.slice(1)} sẽ bị xóa khỏi sơ đồ. Nếu đang có đơn hoạt động, hệ thống sẽ từ chối để bảo vệ dữ liệu vận hành.`,
                            `Xóa ${resourceName}`,
                        )) return;
                        await runButtonAction(deleteBtn, async () => {
                            try {
            ...
            btn.onclick = () => {
                ...
                openModal({
                    ...
                    onReady(subModal, subClose) {
                        subModal.classList.add('admin-resource-modal');
                        $('#confirm-add-admin-resource', subModal).onclick = async event => {
                            ...
                            await runButtonAction(addButton, async () => {
                                try {
    ...
    const toolbar = `<div class="admin-map-head-actions">
        ...
        <button class="button primary small admin-add-resource-button" id="admin-add-resource-btn" type="button">
